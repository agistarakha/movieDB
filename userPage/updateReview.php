...
// $isReviewed = mysqli_query($conn, "SELECT * FROM review WHERE user_id=$userId AND movie_id='$id'");
// if (!empty(mysqli_fetch_assoc($isReviewed))) {

//     header("Location: detail.php?id=" . $id);
// }

$review = query("SELECT * FROM review WHERE user_id=$userId and movie_id = '$id'")[0];
...
if (isset($_POST["submit"])) {
    ...
    $query = "UPDATE review SET score=$score,comment='$comment',posted_date='$date' WHERE user_id=$userId AND movie_id='$id'";
    ...
    <main role="main" class="container-fluid bg-dark-light-ph">
        ...
        <div class="row">
            ...
            <div class="col-8 ml-3">
                ...
                <div class="row">Update Review</div>
                <div class="row">
                    <form method="post" action="">
                        <label for="score">score</label><br>
                        <input type="number" name="score" id="score" placeholder="min:0 max:100" min="0" max="100"
                            value="<?= $review['score'] ?>">
                        <br>
                        <label for="comment">comment</label><br>
                        <textarea name="comment" id="comment" cols="30" rows="10"
                            placeholder="Write comment"><?= $review['comment'] ?></textarea>
                        <br>
                        <button type="submit" name="submit">Submit</button>
                        <br>
                        <br>
                        <a class="btn btn-danger" href="deleteReview.php?id=<?= $movie['movie_id'] ?>">DELETE REVIEW</a>
                    </form>
